...
const images = document.querySelectorAll('.slider__container-line .slider__container-block img');
const sliderLine = document.querySelector('.slider__container-line')
let count = 0;
let width;
function init() {
	console.log('resize');
	...
	images.forEach(item=>{ //массив проходим по всем фотограиям
		item.style.width = 300 +'px'; //каждой задаем ширину как длинна слайдера
		item.style.height =  400 +'px'; // задаем  длинны слайдера
...
function rollSlider() {
	sliderLine.style.transform = 'translate(-'+count*350+'px)';
}